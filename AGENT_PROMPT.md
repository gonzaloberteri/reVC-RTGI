...
Harness files in the game dir (**always write them with absolute paths** — a stale config once made two "night" runs silently re-run noon):

1. `rtgi_autoload.txt` = `1` → auto-loads save 1. Leave this file alone.
2. `rtgi_window.txt` = `1280 720` (or `1920 1080` for showcase, `2560 1440` for perf soak) → windowed background run.
...
4. Launch minimized (`Start-Process -WindowStyle Minimized`), sleep ~90 s, kill `reVC`, read `rtgi.log`, convert `rtgi_shot_*.bmp` → PNG, and **look at the images** (Read tool). Judge like a graphics programmer: silhouettes, light direction, noise, ghosting, brightness vs vanilla art direction. Judge lighting changes from **multiple camera angles** — a single angle once hid whole facades flooding the street pink.
5. Crash triage: early death → `Get-WinEvent` Application log id 1000 for `reVC.exe`; `rtgi.log` shows how far init got.
...
Original backlog #1–#9 all shipped (composites, emissive night models, headlights, real hit albedo, 2nd bounce, SVGF variance guiding, sea reflections, photo mode, checkerboard GI, temporal AO, BLAS compaction, resize/device-lost recreate, rain giblend easing, firefly fix via young-history à-trous bootstrap + radiance clamp 6.0). Config hot-reload just landed. GitHub published with A/B showcase. Details: RTGI.md + git log.

## Backlog (highest value first; when exhausted, invent more and re-polish)

1. **Reflection-pass filtering.** The reflection pass is the only un-filtered RT output: no temporal accumulation, and stochastic foliage/glass (45% coverage) dithers visibly in it. If sparkle persists on car paint in motion, this is the prime suspect. Add temporal reuse (reproject like the GI history) and/or a small edge-aware spatial filter; toggle + timing.
2. **Photo-mode AO accumulation.** Photo mode true-averages GI but AO still shows grain in photo stills — accumulate AO the same way while the camera is still.
3. **Glass & window specular.** Vehicle windshields are non-opaque in the BLAS (dither in reflections) and building windows have no specular response. Give glass a proper reflective treatment (Fresnel-weighted RT reflection sample, deterministic — not stochastic — for smooth surfaces).
4. **Moon shadows + night polish.** Sun shadows exist; at night the moon casts nothing. Trace moon visibility at night hours, soft penumbra, subtle intensity. Re-judge night emissive balance after.
