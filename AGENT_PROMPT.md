...
1. **Water: residual horizon speckle strip + z-order report.** Shader-only water SHIPPED (see RTGI.md). REMAINING: a thin orange-dash texel-grid strip right under the horizon over open sea. Eliminated hypotheses (do NOT retry blindly): CWaterLevel geometry (red-paint test - strip lies beyond ALL of it even at 3x draw dist), world pipe + gloss pipe + stock matFX pipe (strip textures ARE being stripped there - log proves it), sun reflection quad, CClouds horizon rects, translucency show-through (far water now opaque). It also exists in VANILLA captures - some other draw owns it (im2d? another im3d flush outside the wraps? big-building path?). Best attacked LIVE over VNC with debug-menu toggles, not blind capture cycles. Also the original z-order report is still unreproduced.
